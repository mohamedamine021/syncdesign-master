    ...
    <StepLayout stepNumber={7} title="Rotor & Pôles" description="Dimensionnement des pôles et de la culasse rotorique">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          <FormulaResult label="Arc polaire" tex={`b_p = \\alpha_p \\cdot \\tau = ${dim.alphap} \\times ${fmt(dim.tau,1)} = ${fmt(rotor.bp,1)} \\; cm`} result={fmt(rotor.bp, 1)} unit="cm" />
          <FormulaResult label="Hauteur épanouissement" tex={`h_p = h' + R_p - \\sqrt{R_p^2 - (b_p/2)^2} = ${fmt(rotor.hp, 1)} \\; cm`} result={fmt(rotor.hp, 1)} unit="cm" />
          <FormulaResult label="Coefficient de dispersion" tex={`\\sigma_n = 1 + K_\\sigma \\frac{35\\delta}{\\tau^2} = ${fmt(rotor.sigmaN, 3)}`} result={fmt(rotor.sigmaN, 3)} />
